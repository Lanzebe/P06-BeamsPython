...
class BeamElement():
    ...
    def EvaluateAngleEquation(self, Position):
        ...
        for i in self.LoadCase.Elements:
            if i == self:
                RHS = RHS - ((self.GradientQ/24)*((Position-self.StartPos)**4) + (self.Y_InterceptQ/6)*((Position-self.StartPos)**3))
                break
            else:
                RHS = RHS - (((i.GradientQ/24)*((Position-i.StartPos)**4) + (i.Y_InterceptQ/6)*((Position-i.StartPos)**3)) - ((i.GradientQ/24)*((Position-i.EndPos)**4) + (i.Y_InterceptQ/6)*((Position-i.EndPos)**3)))
    ...
    def EvaluateDeflectionEquation(self, Position):
        ...
        for i in self.LoadCase.Elements:
            if i == self:
                RHS = RHS - ((self.GradientQ/120)*((Position-self.StartPos)**5) + (self.Y_InterceptQ/24)*((Position-self.StartPos)**4))
                break
            else:
                RHS = RHS - (((i.GradientQ/120)*((Position-i.StartPos)**5) + (i.Y_InterceptQ/24)*((Position-i.StartPos)**4)) - ((i.GradientQ/120)*((Position-i.EndPos)**5) + (i.Y_InterceptQ/24)*((Position-i.EndPos)**4)))
    ...
    def ShearForceFunction(self,xs):
        ...
        for i in self.LoadCase.Elements:
            if i == self:
                ForcePresent = ForcePresent + (self.GradientQ/2)*(xs-self.StartPos)**2 + self.Y_InterceptQ*(xs-self.StartPos)
                break
            else:
                ForcePresent = ForcePresent + ((i.GradientQ/2)*(xs-i.StartPos)**2 + i.Y_InterceptQ*(xs-i.StartPos)) - ((i.GradientQ/2)*(xs-i.EndPos)**2 + i.Y_InterceptQ*(xs-i.EndPos))
    ...
    def BendingMomentFunction(self,xs):
        ...
        for i in self.LoadCase.Elements:
            if i == self:
                MomentPresent = MomentPresent + (self.GradientQ/6)*(xs-self.StartPos)**3 + (self.Y_InterceptQ/2)*(xs-self.StartPos)**2
                break
            else:
                MomentPresent = MomentPresent + ((i.GradientQ/6)*(xs-i.StartPos)**3 + (i.Y_InterceptQ/2)*(xs-i.StartPos)**2) - ((i.GradientQ/6)*(xs-i.EndPos)**3 + (i.Y_InterceptQ/2)*(xs-i.EndPos)**2)
    ...
    def AngleThetaFunction(self,xs):
        ...
        for i in self.LoadCase.Elements:
            if i == self:
                ThetaPresent = ThetaPresent + ((self.GradientQ/24)*(xs-self.StartPos)**4 + (self.Y_InterceptQ/6)*(xs-self.StartPos)**3)
                break
            else:
                ThetaPresent = ThetaPresent + ((i.GradientQ/24)*(xs-i.StartPos)**4 + (i.Y_InterceptQ/6)*(xs-i.StartPos)**3) - ((i.GradientQ/24)*(xs-i.EndPos)**4 + (i.Y_InterceptQ/6)*(xs-i.EndPos)**3)
    ...
    def DisplacementFunction(self,xs):
        ...
        for i in self.LoadCase.Elements:
            if i == self:
                DisplacePresent = DisplacePresent + ((self.GradientQ/120)*(xs-self.StartPos)**5 + (self.Y_InterceptQ/24)*(xs-self.StartPos)**4)
                break
            else:
                DisplacePresent = DisplacePresent + ((i.GradientQ/120)*(xs-i.StartPos)**5 + (i.Y_InterceptQ/24)*(xs-i.StartPos)**4) - ((i.GradientQ/120)*(xs-i.EndPos)**5 + (i.Y_InterceptQ/24)*(xs-i.EndPos)**4)
