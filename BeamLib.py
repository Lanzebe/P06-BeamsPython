...
class LoadCase():
    def __init__(self,LoadCaseName,LabEnvironment) -> None:
        ...
        self.C1 = UnknownConstant(self)
        self.C2 = UnknownConstant(self)
...
class ReactionForce():
    def __init__(self,ForceName,Position,LoadCase) -> None:
        self.Magnitude = UnknownConstant(LoadCase)
        self.Name = ForceName
        self.Position = Position
...
class BeamElement():
    ...
    def EvaluateAngleEquation(self, Position):
        ...
        RHS = -(self.GradientQ/24)*(Position**4) - (self.Y_InterceptQ/6)*(Position**3)
    ...
    def EvaluateDeflectionEquation(self, Position):
        ...
        RHS = -(self.GradientQ/120)*(Position**5) - (self.Y_InterceptQ/24)*(Position**4)
    ...
    def ShearForceFunction(self,xs):
        ...
        ## Distributed loading:
        ForcePresent = (self.GradientQ/2)*xs**2 + self.Y_InterceptQ*xs
    ...
    def BendingMomentFunction(self,xs):
        ...
        ## Distributed loading:
        MomentPresent = (self.GradientQ/6)*xs**3 + (self.Y_InterceptQ/2)*xs**2
    ...
    def AngleThetaFunction(self,xs):
        ...
        ## Distributed loading:
        ThetaPresent = ((self.GradientQ/24)*xs**4 + (self.Y_InterceptQ/6)*xs**3)
        for force in self.LoadCase.Forces:
            CurrentForce = self.LoadCase.Forces[force]
            if CurrentForce.Position <= self.StartPos:
                if type(CurrentForce) == Force:
    ...
    def DisplacementFunction(self,xs):
        ...
        ## Distributed loading:
        DisplacePresent = ((self.GradientQ/120)*xs**5 + (self.Y_InterceptQ/24)*xs**4)
        for force in self.LoadCase.Forces:
            CurrentForce = self.LoadCase.Forces[force]
            if CurrentForce.Position <= self.StartPos:
                if type(CurrentForce) == Force:
